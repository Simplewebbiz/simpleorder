    ...
    <AdminLayout page-title="Settings">
        <form @submit.prevent="save" enctype="multipart/form-data">
            <div class="settings-grid">
                ...
                <div class="card">
                    <div class="card-header">Storefront Hero Image</div>
                    <div class="hero-preview" v-if="heroPreview || currentHero" :style="{ backgroundImage: 'url(' + (heroPreview || currentHero) + ')' }">
                        <button type="button" class="remove-logo" @click="removeHero">Remove Image</button>
                    </div>
                    <div class="upload-zone" v-else @click="$refs.heroInput.click()" @dragover.prevent @drop.prevent="onHeroDrop">
                        <div class="upload-icon">Image</div>
                        <div class="upload-text">Click or drag to upload hero image</div>
                        <div class="upload-sub">Shown behind the restaurant name and Order Now button</div>
                    </div>
                    <input ref="heroInput" type="file" accept="image/*" hidden @change="onHeroChange" />
                    <div class="field-group" style="margin-top:12px">
                        <label class="field-label">Or enter image URL directly</label>
                        <input v-model="form.hero_image_url" type="text" class="field-input" placeholder="/tenant-media/... or https://..." />
                    </div>
                </div>
                ...
                <div class="card">
                    ...
                    <div class="toggle-row">
                        <label class="toggle-label">Allow Takeout / Pickup</label>
                        <label class="toggle">
                    ...
                    <p class="field-help">Turn delivery off for restaurants that only offer takeout.</p>
...
const currentHero = ref(props.settings?.hero_image_url || null)
...
.hero-preview { min-height: 220px; border-radius: 8px; background-size: cover; background-position: center; display: flex; align-items: flex-end; justify-content: flex-end; padding: 14px; margin-bottom: 16px; border: 1px solid #e5e7eb; }
...
.field-help { color: #6b7280; font-size: 12px; line-height: 1.5; margin: 10px 0 14px; }
